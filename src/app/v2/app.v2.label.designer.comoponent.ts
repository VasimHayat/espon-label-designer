    ...
    sections.forEach((section, index) => {
      switch(section.id) {
        ...
        case 'customer':
          ...
          break;
        case 'metrics':
          payload += `${this.getEscPosAlign('itemIndex')}\n${this.getEscPosBoldText('itemIndex', (data.labelItem || '') + ' ' + data.itemIndex + ' of ' + data.itemCount)}\n${this.getEscPosAlign('createdTime')}\n${this.getEscPosBoldText('createdTime', (data.labelTime || '') + ' ' + data.createdTime)}\n${this.getEscPosAlign('createdDate')}\n${this.getEscPosBoldText('createdDate', (data.labelDate || '') + ' ' + data.createdDate)}\n`;
          break;
        case 'branding':
          payload += `${this.getEscPosAlign('branding')}\n${this.getEscPosBoldText('branding', data.branding)}\n`;
          break;
        case 'subtotals':
          payload += `${this.getEscPosAlign('total')}\n${this.getEscPosBoldText('total', (data.labelSubtotal || '') + ' $' + this.formatCurrency(data.subtotal) + '\n' + (data.labelTax || '') + ' $' + this.formatCurrency(data.tax) + '\n' + (data.labelDelivery || '') + ' $' + this.formatCurrency(data.deliveryFee))}\n`;
          break;
        case 'total':
          payload += `${this.getEscPosAlign('total')}\n[BIG FONT ON]\n${this.getEscPosBoldText('total', (data.labelTotal || '') + ' $' + this.formatCurrency(total))}\n[BIG FONT OFF]\n`;
          break;
        case 'provider':
        ...
        case 'items':
          ...
          break;
        case 'orderClass':
          payload += `${this.getEscPosAlign('orderClass')}\n${this.getEscPosBoldText('orderClass', (data.labelOrderType || '') + ' ' + data.orderClass)}\n`;
          break;
      }
